<div class="container mx-auto px-4 py-8">
    ...
    @if($wishlist->count() > 0)
        <div class=" overflow-x-auto bg-white rounded-lg shadow-sm py-3 px-6" >
            <div class="flex justify-between items-center py-4 px-6 border-b" style="min-width: 700px;" >
            ...
            <div class="divide-y" style="min-width: 700px;">
                <!-- Table Header -->
                                ...
                                <div class="col-span-6 flex items-center space-x-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500">
                                            {{ $item->formation->etablissement->nom ?? null }}
                                        </p>
                                        <div class="flex items-center mt-1">
                                            
                                            @if($item->formation->reviews->count() > 0)
                                            <svg class="w-4 h-4 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
                                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118l-2.8-2.034c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                            </svg>
                                                <span class="ml-1 text-sm text-gray-600">{{ number_format($item->formation->reviews->avg('note'), 1) }}</span>
                                                <span class="ml-2 text-xs text-gray-400">({{ $item->formation->reviews->count()  }} avis)</span>
                                            @endif
                                        </div>
                                    </div>
                                </div>
